...
export class NewStepDialogUiComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.newStepForm = this.formBuilder.group({
      ...
      batchSize: [(this.step && this.step.batchSize) ? this.step.batchSize || 100 : this.flow.batchSize, CustomFieldValidator.number({min: 1})],
      threadCount: [(this.step && this.step.threadCount) ? this.step.threadCount || 4 : this.flow.threadCount, CustomFieldValidator.number({min: 1})],
